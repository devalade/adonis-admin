...
function modelValuesPrefix(modelName) {
    return string
        .snakeCase(modelName)
        .replace(/_signups?$/, '')
        .replace(/_sessions?$/, '');
}
async function resolveValuesImport(appRoot, modelName, columnName) {
    ...
    const pluralConstant = source.match(/export const ([A-Z0-9_]+)\s*=\s*\[[\s\S]*?\]\s*as const/)?.[1];
    if (pluralConstant) {
        return {
...
function classifyColumnKind(name, column) {
    if (column?.isPrimary) {
        return 'primaryKey';
    }
...
function iconForModel(modelName) {
...
export async function introspectModel(Model, appRoot) {
    Model.boot();
    const modelName = Model.name;
    const columns = [];
    for (const name of Model.$columns) {
        const definition = Model.$getColumn(name);
        const kind = classifyColumnKind(name, definition);
        if (kind === 'password') {
            continue;
        }
        const valuesImport = kind === 'enum' ? await resolveValuesImport(appRoot, modelName, name) : undefined;
        const includeInTable = kind !== 'foreignKey';
        const includeInForm = !SKIP_FORM_NAMES.has(name) &&
            kind !== 'primaryKey' &&
            kind !== 'timestamp' &&
            kind !== 'foreignKey';
        columns.push({
