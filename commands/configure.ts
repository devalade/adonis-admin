...
async function writeStub(stubPath: string, destination: string, force: boolean) {
...
async function copyStubDirectory(stubDir: string, destinationDir: string, force: boolean) {
  const published: string[] = []

  async function walk(currentStubDir: string, currentDestinationDir: string) {
...
async function patchAdonisRc(appRoot: string) {
...
async function patchKernel(appRoot: string) {
...
export default class ConfigureAdmin extends BaseCommand {
  ...
  async run() {
    ...
    if (
      await writeStub(
        join(stubs, 'config/admin.stub'),
        join(appRoot, 'config/admin.ts'),
        this.force
      )
    ) {
    ...
    if (
      await writeStub(
        join(stubs, 'app/admin/register.stub'),
        join(appRoot, 'app/admin/register.ts'),
        this.force
      )
    ) {
    ...
    if (
      await writeStub(
        join(stubs, 'start/admin.stub'),
        join(appRoot, 'start/admin.ts'),
        this.force
      )
    ) {
    ...
    if (
      await writeStub(
        join(stubs, 'providers/admin_provider.stub'),
        join(appRoot, 'providers/admin_provider.ts'),
        this.force
      )
    ) {
    ...
    const inertiaPublished = await copyStubDirectory(
      join(stubs, 'inertia/admin'),
      join(appRoot, 'inertia/admin'),
      this.force
    )
    published.push(...inertiaPublished.map((file) => `inertia/admin/${file}`))

    const pagesPublished = await copyStubDirectory(
      join(stubs, 'inertia/pages/admin'),
      join(appRoot, 'inertia/pages/admin'),
      this.force
    )
